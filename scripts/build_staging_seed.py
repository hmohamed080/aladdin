...
def docs_markdown(accounts: list[Account]) -> str:
    ...
    lines += [
        ...
        "",
        "Point-in-time facts confirmed against hosted `aladdin-staging` at 56/56 migration",
        "parity (2026-09-07). Re-verify with the hosted verifier command below rather than",
        "trusting these numbers indefinitely — they will drift as the demo world grows.",
        "",
        "- **Admin platform totals at last verification:** 21 RFQs · 17 quotations · 12 orders",
        "  · 7 projects.",
        "- **Karim Adel (`karim-branch-sales`) is healthy.** His membership carries",
        "  `sales.read` and `sales.write`, scoped to the Cairo branch. An earlier static-code",
        "  read (before hosted verification existed) assumed he only held the retired",
        "  `sales.opportunity.read` / `sales.opportunity.write` capability keys and would see",
        ...
        "  only `org.manage`'s absence is meaningful, and that absence is the point.",
        "- **Hosted staging may carry unrelated non-demo registrations** — accounts created",
        "  outside this manifest, plus at least one external organization. Hosted",
        "  verification identifies and validates the 26 demo identities above by their",
        "  specific UUIDs — it does not require them to be the only rows in the database,",
        "  and it never treats an unrelated registration as a 27th demo account. This file",
        "  never names or otherwise identifies those unrelated registrations.",
        "- **The Jobs / Points / Portfolio / Certificates / Reviews / Installer Network",
        "  domains currently hold no hosted demo data.** This is expected current coverage,",
        "  not a product defect — those migrations are schema-only; no seed or enrichment",
        "  layer populates them yet.",
        "",
        "## Verifying",
        "",
        "Fixture/rehearsal mode (strict — requires exactly the 26 demo accounts and nothing",
        "else):",
        "",
        "```bash",
        ...
        "```",
        "",
        "Hosted mode (tolerates unrelated non-demo registrations, still strictly validates",
        "the 26 expected demo identities by UUID) — the one supported command:",
        "",
        "```bash",
        "python scripts/verify_staging_seed.py",
        "```",
        "",
        "Both are read-only and wrapped in a transaction that always rolls back. Each checks",
        "population, address uniqueness and deliverability, persona/membership/branch",
        "linkage, commerce totals against their own line items, and — impersonating all 26",
        "accounts under RLS — the landing route and non-emptiness of every one of them.",
        "",
        "## Related",
        "",
