...
import { PackageIcon, BookmarkIcon, BadgeCheckIcon } from "@/components/ui/icons";
 ...
 *
 * Each card offers exactly two moves: open the product, or request a price for it.
 * There is deliberately NO cart and NO price on the card — a saved product is a
 * shortlist entry, and in a consultation-first product the price comes from a real
 * quote against a real quantity, not from a shelf label.
 */
          ...
          <Card className="flex h-full flex-col gap-sm">
            <div className="flex items-start justify-between gap-sm">
              ...
              <div className="flex items-center gap-2">
                {p.category ? <Badge tone="neutral">{m.commerce.categories[p.category]}</Badge> : null}
                {p.product_id ? (
