...
export type LandingDetail = "videos" | "partners" | number;
...
export function LandingDetails({ detail, onClose }: { detail: LandingDetail | null; onClose: () => void }) {
  ...
  const title = role?.title ?? (detail === "videos" ? t.videoAction : t.partnerAction);
  return <dialog ref={ref} className={styles.dialog} dir={dir} aria-labelledby="landing-detail-title" onClose={onClose}
    ...
    <div className={styles.panel}>
      ...
      {detail === "videos" ? <><p>{locale === "ar" ? "لم تُنشر فيديوهات معتمدة بعد. هذه الأقسام مخصصة للمحتوى القادم." : "No approved videos have been published yet. These categories are reserved for upcoming content."}</p><ul className={styles.features}>{t.videoCategories.map((category) => <li key={category}>{category} — {t.videoPending}</li>)}</ul></> : null}
