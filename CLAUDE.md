...
- **Business classification belongs to the Organization:** Showroom/Dealer · Supplier · Manufacturer · Importer · Wholesaler · contractor company · design/engineering office are canonically **`organizations.org_type`**, never a person's long-term identity. **`users.primary_account_type` is personal persona state only** — one user may own organizations of several different types at once (*Ahmed*: persona Engineer · *AH Showroom* `showroom_dealer` · *AH Import* `importer`, same user ID), which one value cannot represent. Picking *"I am a Showroom"* at registration stays — it means *"I am creating a business whose `org_type` is X"*. Business-valued `primary_account_type` values are **transitional debt**, not the target source of truth; never mirror `org_type` into `users`.
