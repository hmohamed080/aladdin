...
| | |
|---|---|
| **Version** | Runtime snapshot · 2026-08-16 (staging deployment readiness) |
| **Owner** | Foundation / Operations |
| **Last updated** | 2026-08-16 |
| **Updated by** | Claude — first cloud STAGING deployment readiness (deployment-only; no product change) |
| **Current focus** | **Showroom MVP Completeness (Sprint 14):** the B2B workspace was reorganized around its primary account. The eleven-item flat nav became **five capability-derived sections** (Overview · Buying · Network · Selling · Business), and the four pages that stacked the buy side and the sell side in one scroll — RFQs, Quotations, Orders, Projects — each now state **one perspective**, with the opposite side as a tab shown only to an organization holding that role. Three nav keys were renamed (`rfqs`→`purchaseRequests`, `quotations`→`offers`, `organization`→`team`); **route paths are unchanged**. Six modules were added: **Suppliers**, **Institutions**, **Technicians**, **Saved products**, **Reports & analytics**, **Settings**; the dashboard was rebuilt buyer-first with a "What do you want to do today?" action ramp; Projects gained real KPIs. One canonical UI set now backs every list (`data-table`, `stat-tiles`+`TabLinks`, `filter-bar`, `breakdown`). Mobile gained a **More sheet** so all seventeen modules stay reachable on a phone. One migration (`20260816090001`): `saved_products` + `save_product`/`unsave_product` RPCs + `saved_product_list` view, and the `persona` column on the professional directory (the two pgTAP approved-column guards were updated deliberately). Reference images live at `UI-UX/references/showroom/`, used for structure only — points/rewards, add-to-cart, ratings and supplier CRUD were explicitly **not** copied because they contradict the approved direction. Branch `feature/showroom-mvp-completeness`; no `.pen` change. |
...
| **Current Branch** | `chore/staging-deployment-readiness` (created from `main` @ `944e954`) |
