...
| **Updated by** | Claude Code (Opus 4.8) — Phase 2, Sprint 6 Sales Ownership, Realtime & Performance Hardening (ownership RPCs + scoped Postgres-Changes Realtime + executed E2E/visual-QA/perf gates) |
...
| **Implementation Status** | **Sprint 6 validated (frontend typecheck/lint/125 tests/build; backend ruff+pytest; Supabase two clean cycles reset+lint+416 pgTAP; 6 two-session races; Playwright E2E executed & green incl. two-context Realtime; executed visual-QA matrix; production perf measured; dev+prod runtime smoke).** Adds post-create **ownership** edits — `set_customer_ownership` (branch/assignee) and `set_lead_source_branch` (source/branch/compatible-reassign), both version/updated_at-guarded, scope+capability enforced, strand-rejecting, transactionally audited; **`customer_type` immutable** (no domain approval); lead lifecycle out of bounds for the lead RPC. Adds **scoped Realtime** (Postgres Changes on `leads`+`follow_up_tasks`; refresh-only client, RLS-scoped, defers on edit, rebuilds on org/branch change). Visual QA fixed a 360px cockpit overflow. Sprint 5.1 merged (PR #8, `5a47011`). Branch `feature/sales-ownership-realtime`, PR to `main` pending. No products/RFQ/quotes/projects/B2C/payments/WhatsApp/OCR/AI. |
