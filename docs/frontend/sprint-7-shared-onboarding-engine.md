...
## E2E harness determinism (follow-up)

The standard `pnpm --filter frontend e2e` command previously ran against `next dev`, whose on-demand route compilation stalled under sustained full-suite load and produced non-deterministic navigation timeouts. The Playwright `webServer` now runs a **built production server** (`next build && next start`, `reuseExistingServer: false`, the realtime-debug flag inlined at build) — no manual server, the command is unchanged. Two remaining sources of flake were fixed without weakening any assertion: production exposed a real bug where the language switch didn't flip `<html dir>` (React doesn't patch `<html>` attributes on client re-render — fixed to mirror the theme switch), and all `waitForURL` waits now resolve on `commit` instead of the `load` event (which lags far behind an interactive page under load); `retries: 2` absorbs the inherently timing-sensitive multi-context realtime scenarios. Result — the standard command run twice back-to-back from a clean state: **run 1 = 47 passed / 1 flaky / 42 skipped / 0 failed / exit 0**; **run 2 (consecutive, no manual cleanup) = 46 passed / 2 flaky / 42 skipped / 0 failed / exit 0** (the "flaky" tests passed on retry; zero failed on both).
