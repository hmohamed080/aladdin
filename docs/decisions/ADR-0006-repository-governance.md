...
- **2026-08-23 (branch protection actually enforced; one temporary exception):** the protection this ADR describes was **documented but never configured on GitHub** — the API reported `main` as *"Branch not protected"*, and the repository additionally still permitted **squash and rebase merges**, directly contradicting the merge strategy above. Both are now enforced: PR required, required checks `frontend` · `backend` · `docs` · `supabase-rls`, force-pushes and branch deletion blocked, `required_linear_history` deliberately **off** (GitHub's linear-history rule *forbids* merge commits and would invert this ADR's `--no-ff` requirement), and repository merge strategy restricted to **merge commits only**. Required approvals = **1**, which is this ADR's existing `≥1 review` rule, not a new one.

  **Temporary exception — `enforce_admins` is disabled.** GitHub does not permit self-approval, and this repository currently has a **single maintainer** ("until then, the maintainer reviews all PRs", *Code ownership* above). With admin enforcement on, the 1-approval requirement would deadlock the merge flow entirely: the sole maintainer could neither approve nor merge their own PR. The rules therefore bind normal contributors while the maintainer retains an admin bypass.

  This is an **enforcement gap, not a change of decision** — the governance above stands unamended in substance. **Revisit when a second reviewer or a team exists**, at which point `enforce_admins` should be enabled and the `CODEOWNERS` file this ADR already flags as *"recommended, not yet created (⚑ add when a team exists)"* should land with it. Until then the ADR's review requirement is **partially self-enforced by convention** for the maintainer.
