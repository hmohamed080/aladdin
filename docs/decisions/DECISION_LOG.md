...
| [0007](ADR-0007-identity-and-tenancy-model.md) | Identity & Tenancy Implementation Model (Phase 1) | Accepted | 2026-08-02 | RLS helpers as `security definer` (not JWT claims yet); branch access = home branch + `membership_branch_access` join + capability-derived org-wide; profile bootstrap via `auth.users` trigger; platform admin only in `platform_role_grants` (no user write path); explicit grants + RLS-with-policy on every table. Refines 0002 for the identity/tenancy tables. | **Active** — implemented on `feature/identity-multitenancy`; JWT custom-claim optimization deferred (TECHNICAL_DEBT). |
