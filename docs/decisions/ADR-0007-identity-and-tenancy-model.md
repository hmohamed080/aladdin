...
- **D15 — membership & branch write-path RPCs** (`membership_invite/activate/set_capabilities/suspend/revoke`, `branch_assign/unassign`): authority from `app.has_capability(org,'org.members.manage'|'branch.manage')`; **no-escalation** (a manager may only grant capabilities they themselves hold); **last-owner protection** (`app.assert_not_last_owner` locks the org's active `org.manage` rows and blocks removing the last one) — implemented, not deferred; **cross-tenant assignment impossible** (branch org must equal membership org); duplicates rejected by `uq_memberships_user_org` / `uq_membership_branch_access`. `primary_branch_id` still grants no access.
...
- **Concurrency/idempotency.** One open verification per user (partial unique index); single-apply (`applied_at` + row lock); duplicate membership/branch rejected by unique constraints; last-owner serialized by row locks; every RPC is transactional so an audit-insert failure rolls the whole action back.
