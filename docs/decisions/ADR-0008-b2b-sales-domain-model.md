...
## Amendment — Sprint 6: ownership edits + scoped Realtime (2026-08-05)

Migration `20260806090001_sales_ownership_and_realtime.sql` (forward-only) extends the trusted-write pattern and adds a Realtime boundary. No prior decision is reversed.

- **Post-create ownership RPCs.** `set_customer_ownership(customer, expected_updated_at, [branch], [assignee])` and `set_lead_source_branch(lead, expected_version, [source], [branch], [reassign])` — both `security definer`, `search_path=""`, `authenticated`-only, caller derived from `auth.uid()`. They enforce active-org membership + branch scope, require `sales.assign` for a branch move/reassignment (source-only needs `sales.write`), keep the effective assignee **branch-compatible** (a move that would strand the current assignee is rejected — never a silent unassign), reject cross-tenant branches, and emit `customer.reassigned` / `lead.details_changed` audit events in the same transaction (none on failure/conflict). Optimistic concurrency reuses the existing tokens (customer `updated_at`, lead `version`) → `40001`.
- **Lead lifecycle stays isolated.** `set_lead_source_branch` cannot touch `status/stage/won/lost/closed_at/lost_reason` (structurally — not parameters); lifecycle remains `transition_lead`-only.
- **`customer_type` is immutable.** No product/domain document approves changing it, so it is intentionally not editable (correct a mistype by archive + re-create). Revisit only via a documented product rule.
- **Scoped Realtime = Postgres Changes (Broadcast rejected).** Publishes **exactly** `leads` + `follow_up_tasks` to `supabase_realtime`; RLS authorizes every change against the subscriber's own `SELECT` policy. Chosen over Broadcast-from-database because it is RLS-native with zero extra schema for pilot volume; Broadcast (triggers + `realtime.messages` RLS) is the future upgrade if per-row authorization volume grows. The browser client subscribes with the caller's token via `realtime.setAuth`, filtered to the server-derived active org, and is **refresh-only** — it never renders a payload; a change only triggers an RLS-scoped server refetch, so it cannot surface unauthorized rows and duplicate/out-of-order events cannot corrupt UI state. No identity/verification/audit/customer-PII table is published; no service-role key reaches the browser.
