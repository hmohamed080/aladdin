...
- **Business classification is an organization property.** Showroom/Dealer · Supplier · Manufacturer · Importer · Wholesaler · contractor company · design/engineering office are canonically **`organizations.org_type`**; **`users.primary_account_type` carries personal persona state only** and is never a mirror of `org_type` — one user may own organizations of several different types at once. Business-valued account types in the current enum are **transitional** (unchanged here; migration owned by the upcoming Account & Workspace Model feature — [`../technical/TECHNICAL_DEBT.md`](../technical/TECHNICAL_DEBT.md)).
