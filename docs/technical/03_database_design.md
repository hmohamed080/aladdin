...
- Written by constrained postgres-owned workflow functions in the same transaction as the protected mutation; direct `service_role` audit INSERT is prohibited because it could spoof actor, tenant, subject, or metadata. Readable by admins per [06](06_rls_strategy.md).
