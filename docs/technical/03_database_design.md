...
> **Implementation status (Phase 1 + Sprint 1.1 security review, 2026-08-02):** §1 Identity, §2 Organizations & access, and §12 `audit_log` are **implemented** (migrations `20260802090001–3`). Deltas from this spec (see **[ADR-0007](../decisions/ADR-0007-identity-and-tenancy-model.md)** and [`../database/phase1-identity-tenancy-review.md`](../database/phase1-identity-tenancy-review.md)): a `membership_branch_access` join table for multi-branch assignment; `memberships.branch_id` is implemented as **`primary_branch_id`** (descriptive only — branch authority comes from `membership_branch_access`); `organizations` gained `slug`/`primary_locale` + a slug-format `CHECK`; **`administrator` was removed from the `account_type` enum** (platform authority is only `platform_role_grants`); public discovery is via curated views (`organization_public_directory`, `profile_public_directory`), not the base tables. **`users.primary_account_type` is server-controlled** (not in the authenticated update grant — it changes only via the approved upgrade/admin workflow), and `profiles` gained a server-controlled **`public_profile_status`** (`hidden`/`listed`) that gates public discovery (a professional account type alone does not make a profile public — Sprint 1.2 fix). **Sprint 2 (2026-08-03):** §3 Verification is **implemented** (`verifications` + minimal `verification_documents`, migration `20260803090001`) with `requested_account_type`/`grants_public_listing`/`applied_at` extensions; all account-upgrade/membership/branch write paths are `security definer` RPCs (migrations `2026080309000x`), and the constrained `app.record_audit_event()` writer now exists. Remaining sections stay specification until their feature migration lands.
