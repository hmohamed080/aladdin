...
- **Organization/profile discovery (Sprint 4.2 hardening).** `organization_public_directory` and `profile_public_directory` are `security_invoker = true` views over constrained `security definer` readers `app._organization_public_directory()` / `app._profile_public_directory()` (`search_path=''`, schema-qualified, in the non-exposed `app` schema, `PUBLIC` execute revoked, `EXECUTE` to `anon`/`authenticated`/`service_role`). This clears the Advisor "Security Definer View" finding while keeping the base `organizations`/`profiles`/`users` tables **private to `anon`** (no base-table grant) — the reader, not a broadened grant, performs the curated read. Directory objects grant `SELECT` only (no `TRUNCATE`/`REFERENCES`/`TRIGGER`). See [ADR-0007 D21](../decisions/ADR-0007-identity-and-tenancy-model.md).
