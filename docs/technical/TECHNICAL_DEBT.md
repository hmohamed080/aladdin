...
| **Business-valued `account_type` / `primary_account_type` (transitional)** — `showroom_dealer`, `supplier`, `manufacturer`, `importer`, `wholesaler` exist in the `account_type` enum, and registration/onboarding paths may still set them on the user. In the **target** model, business classification is canonically **`organizations.org_type`** and `users.primary_account_type` carries **personal persona state only** — one user may own several organizations of different types, which one value cannot represent (PRODUCT_DIRECTION_GUIDE *Business Classification Belongs to the Organization*) | **Not a second source of truth yet** — `org_type` is already canonical for organizations; the risk is future code reading a business-valued `primary_account_type` as the user's identity, or mirroring `org_type` into `users` | **Upcoming Account & Workspace Model feature**: audit every read/write of business-valued `primary_account_type`, then migrate to organization-owned classification behind a reviewed migration. **Do not** change the enum, migration behaviour, or these paths ahead of that feature, and **never** mirror `org_type` into `users` |
