...
-- || 'literal'` appeared (11 occurrences).
--
-- SECOND FIX, same function, found by calling it as a freshly-registered
-- Tradesperson against the real database: the professional block always
-- required a `user_activities` row, but public.activities seeds persona
-- subtypes ONLY for engineer and sales. An installer_technician (whose
-- subtype IS the 14-trade taxonomy, public.user_trades), a contractor or an
-- interior_designer could therefore never satisfy 'activities' and never reach
-- 100% — the "Complete your profile" card could never disappear for them. The
-- criterion now counts only when it is satisfiable: a Tradesperson satisfies it
-- with at least one ACTIVE held trade; a persona with active catalog activities
-- satisfies it with a user_activities row; a persona with neither is not asked.
--
-- THIRD FIX: 'locality' is dropped from the all-audience criteria. Nothing in
-- this codebase writes profiles.locality_id — no RPC, no UI, and no locality
-- vocabulary table for it to reference (the column has no FK). Counting it
-- made 100% unreachable for EVERY caller, so the card could never disappear.
-- Re-add it in the same change that ships a real locality write path.
-- Everything else is unchanged from 20260924090008_profile_completion.sql.
-- ===========================================================================
...
begin
  ...
  if v_persona is not null and v_persona in
    ...
    -- 'activities' only when this persona actually HAS something to pick.
